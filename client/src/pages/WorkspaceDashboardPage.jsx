  ...
  const fetchWorkspaceDashboard = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/workspace-dashboard/${slug}`,
        {
      ...
      console.error("Workspace dashboard fetch failed", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">
            Overview of tasks inside this workspace
          </p>
        </div>

        <Link to={`/workspace/${slug}`} className="btn-secondary">
          ← Back to Workspace
        </Link>
      </div>
